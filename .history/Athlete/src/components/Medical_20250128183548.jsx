...
function App() {
  ...
  const [formData, setFormData] = useState<Omit<Injury, 'id' | 'lastUpdated' | 'prescriptions'>>({
    date: '',
    injuryType: '',
    severity: 'Low',
    status: 'Active',
  ...
  const [prescriptionForm, setPrescriptionForm] = useState<Omit<Prescription, 'id'>>({
    medication: '',
    dosage: '',
    frequency: '',
    startDate: '',
  ...
  const handlePrescriptionSubmit = (injuryId: string) => {
                    ...
                    <div key={injury.id} className="bg-gray-800 rounded-lg shadow-lg p-6 border-l-4 border-blue-500">
                      ...
                      <div className="mt-4">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="text-sm font-medium text-gray-300">Prescriptions</h4>
                          <button
                            onClick={() => setShowPrescriptionForm(injury.id)}
                        ...
                        {injury.prescriptions.length > 0 ? (
                          <div className="space-y-2">
                            {injury.prescriptions.map((prescription) => (
                              <div key={prescription.id} className="bg-gray-700 rounded-md p-3">
                                <div className="flex items-start justify-between">
                                  <div className="flex items-center">
                                    <Pill className="h-4 w-4 text-blue-400 mr-2" />
                                    <span className="text-sm font-medium text-gray-200">{prescription.medication}</span>
                                  </div>
                                  <span className="text-xs text-gray-400">{prescription.dosage}</span>
                                </div>
                                <div className="mt-2 text-xs text-gray-400">
                                  <p>Frequency: {prescription.frequency}</p>
                                  <p>Duration: {new Date(prescription.startDate).toLocaleDateString()} - {new Date(prescription.endDate).toLocaleDateString()}</p>
                                  {prescription.notes && <p className="mt-1 text-gray-500">{prescription.notes}</p>}
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      {showPrescriptionForm === injury.id && (
                        <div className="mt-4 bg-gray-700 rounded-lg p-4">
                          <h4 className="text-sm font-medium text-gray-200 mb-3">Add New Prescription</h4>
                          <div className="space-y-3">
                            <div>
                              <input
                                type="text"
                                placeholder="Medication name"
                                className="w-full bg-gray-600 rounded-md border-gray-500 text-sm"
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <input
                                type="text"
                                placeholder="Dosage"
                                className="bg-gray-600 rounded-md border-gray-500 text-sm"
                            ...
                            <div className="flex justify-end space-x-2">
                              ...
                              <button
                                type="button"
                                onClick={() => handlePrescriptionSubmit(injury.id)}
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  {recoveredInjuries.map((injury) => (
                    <div key={injury.id} className="bg-gray-800 rounded-lg shadow-lg p-6 border-l-4 border-green-500">
                      ...
                      {injury.prescriptions.length > 0 && (
                        <div className="mt-4">
                          <h4 className="text-sm font-medium text-gray-300 mb-2">Prescriptions</h4>
                          <div className="space-y-2">
                            {injury.prescriptions.map((prescription) => (
                              <div key={prescription.id} className="bg-gray-700 rounded-md p-3">
                                <div className="flex items-start justify-between">
